...
static void phex(uint8_t* str) //16진수로 출력
{
//16진수 길이 정의
#if defined(AES256)
    uint8_t len = 32; 
...
// static void test_encrypt_ecb_verbose(void) //상세 정보 출력
// {
...
//     // 128bit key
//     uint8_t key[16] =        { (uint8_t) 0x3b, (uint8_t) 0x7e, (uint8_t) 0x15, (uint8_t) 0x16, (uint8_t) 0x28, (uint8_t) 0xae, (uint8_t) 0xd2, (uint8_t) 0xa6, (uint8_t) 0xab, (uint8_t) 0xf7, (uint8_t) 0x15, (uint8_t) 0x88, (uint8_t) 0x09, (uint8_t) 0xcf, (uint8_t) 0x4f, (uint8_t) 0x3c };
//     // 512bit text
...
static int test_encrypt_ecb(void)
{
    ...
    printf("plaintext: \t");
    for(int i = 0; i < sizeof(in); i++){ //plaintext를 출력
        printf("%.2x", in[i]);
    }
    printf("\n"); //개행 

    AES_init_ctx(&ctx, key);
    AES_ECB_encrypt_buffer(&ctx, in, sizeof(in));

    printf("ECB encrypt: \t");
    
    for(int i = 0; i < sizeof(in); i++){ //encrypted된 값을 출력
        printf("%.2x", in[i]);
    }
    printf("\n\n"); //개행
...
static int test_decrypt_cbc(void)
{
    ...
    for(int i = 0; i < sizeof(in); i++){ //encrypted text 출력
        printf("%.2x", in[i]);
    }
    printf("\n"); //개행
    ...
    for(int i = 0; i < sizeof(in); i++){ //decrypted된 값을 출력
        printf("%.2x", in[i]);
    }
    printf("\n\n"); //개행
...
static int test_encrypt_cbc(void)
{
    ...
    AES_init_ctx_iv(&ctx, key, iv); //KeyExpansion을 통해 RoundKey 생성
    AES_CBC_encrypt_buffer(&ctx, in, sizeof(in)); //in 값들이 encrypted 되어 갱신됨

    printf("CBC encrypt:\t");

    for(int i = 0; i < sizeof(in); i++){ //encrypted 된 값을 출력
        printf("%.2x", in[i]);
    }
    printf("\n\n"); //개행 
...
static int test_decrypt_ecb(void)
{
    ...
    AES_init_ctx(&ctx, key);
    AES_ECB_decrypt_buffer(&ctx, in, sizeof(in));

    printf("ECB decrypt: \t");

    for(int i = 0; i < sizeof(in); i++){ //추가한 코드, decrypted 된 값을 출력하기 위해 만듦
        printf("%.2x", in[i]);
    }
    printf("\n\n"); //개행
